package com.easyapps.focusmode.launcher.utils
...
import com.easyapps.focusmode.launcher.AppInfo
import java.util.*
...
class Utils {
    ...
    companion object {
        ...
        fun getFocusProgress(context: Context): Double {
            ...
            addDateToTiming(endFocus, startFocus)


            if (currentTime.before(startFocus)) {
                return 100.0
            }
            if (currentTime.after(endFocus)) {
                return 100.0
            }
            val progress: Double =
                ((currentTime.time.time.toDouble() - startFocus.time.time) / (endFocus.time.time - startFocus.time.time))
            return progress * 100
        }

        fun addDateToTiming(endFocus: Calendar, startFocus: Calendar): Calendar {
            val currDate = Calendar.getInstance()
            if (endFocus.time < startFocus.time && currDate.time > startFocus.time) {
                val endDate = Calendar.getInstance().get(Calendar.DAY_OF_YEAR) + 1
                if (endDate < 365) {
                    endFocus.set(Calendar.DAY_OF_YEAR, endDate)
                } else {
                    endFocus.set(Calendar.YEAR, currDate.get(Calendar.YEAR) + 1)
                }
            } else if (endFocus.time < startFocus.time && currDate.time < endFocus.time) {
                val startDate = Calendar.getInstance().get(Calendar.DAY_OF_YEAR) - 1
                if (startDate >= 1) {
                    startFocus.set(Calendar.DAY_OF_YEAR, startDate)
                } else {
                    startFocus.set(Calendar.YEAR, currDate.get(Calendar.YEAR) - 1)
                }
            }
            return endFocus
        }
